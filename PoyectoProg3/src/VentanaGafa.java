...
public class VentanaGafa extends JFrame {
	...
	public JLabel lzoom;
	public JLabel langulo;
	public JPanel arribaiz;
	...
	public VentanaGafa(Gafas s) {
		...
		cp.add(abajo);
		browse = new JButton(
				VentanaWeb.resize("browse.jpeg", (int) VentanaWeb.redimx(1500), (int) VentanaWeb.redimy(60)));
		abajo.add(browse);
		abajo.setBounds(0, (int) VentanaWeb.redimy(380), (int) VentanaWeb.redimx(640), (int) VentanaWeb.redimy(60));
		agregar = new JButton(
				VentanaWeb.resize("agregar.jpeg", (int) VentanaWeb.redimx(300), (int) VentanaWeb.redimy(60)));
		cp.add(agregar);
		agregar.setBounds((int) VentanaWeb.redimx(640), (int) VentanaWeb.redimy(380), (int) VentanaWeb.redimx(300),
				(int) VentanaWeb.redimy(60));

		// gafas

		gafas = s.label;
		arriba.add(s.label);
		gafas.setLocation((int) VentanaWeb.redimx(590), (int) VentanaWeb.redimy(80));
		gafas.setSize((int) VentanaWeb.redimx(400), (int) VentanaWeb.redimy(200));
		gafas.zoom = 0.5;

		moverGafas(gafas, arriba);

		Font fuente = new Font("Monospaced", Font.BOLD, 28);
		Font f = new Font("", Font.ITALIC, 18);
		descrip = new JLabel();
		precio = new JLabel();
		descrip.setText("<html>" + s.descripción + "</html>");
		descrip.setFont(f);
		arriba.add(descrip);
		...
		arriba.add(precio);
		descrip.setLocation((int) VentanaWeb.redimx(600), (int) VentanaWeb.redimy(180));
		descrip.setSize((int) VentanaWeb.redimx(300), (int) VentanaWeb.redimy(180));
		precio.setLocation((int) VentanaWeb.redimx(639), (int) VentanaWeb.redimy(250));
		precio.setSize((int) VentanaWeb.redimx(330), (int) VentanaWeb.redimy(160));
		...
		zoom.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				gafas.setSize((int) VentanaWeb.redimx(200 * zoom.getValue() / 50),
						(int) VentanaWeb.redimy(100 * zoom.getValue() / 50));
			}

		});

		langulo = new JLabel("Rotación:");
		lzoom.setSize(40, 5);
		lzoom.setVisible(true);
		arribad.add(langulo);
	...
	public ImageIcon ResizeWindow(String ImagePath) {
		...
		descrip.setLocation(w+(int) VentanaWeb.redimx(10), h- (int) VentanaWeb.redimy(180));
		precio.setLocation(w+(int) VentanaWeb.redimx(70), h-(int) VentanaWeb.redimy(120));
...
//			
			public void run() {
				...
				do {
					...
					if (raton && x <= (arriba.getWidth() - (int) VentanaWeb.redimx(100))
							&& y >= -(int) VentanaWeb.redimy(50) && x >= -(int) VentanaWeb.redimx(100)
							&& y <= (arriba.getHeight()) - (int) VentanaWeb.redimy(50)) {
						l.setLocation(x, y);
					}
